...
//  OneButton Socket
//
//  Created by Aaron Jacobs on 2005-06-21.
...
/*
	...
	the inspiration for this class and its method of listening for connections
	and handling communication with a delegate.
*/
...
// The socket successfully connected to the host provided to
// connectToHost:port:.
...
// An accepting socket spun off a new socket for an accepted connection.
// newSocket is autoreleased, so remember to retain it.  newSocket will call
// socket:didConnectToHost:port: once it has successfully set up the connection.
- (void)socket:(ONBSocket *)socket
...
// The socket disconnected in some error state.  remainingData contains any data
// that came in over the socket before it disconnected but had not yet been used
// to fill a read request.  After this delegate method is called, the socket is
// useless and should be released.
- (void)socket:(ONBSocket *)socket
...
@interface ONBSocket : NSObject
{
	...
	***************************************************************/
	...
	// The CFSocket responsible for accepting new connections and associated
	// objects.
...
// The delegate is a weak reference.  Be sure to change it if the delegate gets
// released.  Note that if you change the delegate while read and write requests
// are pending, the new delegate may receive completion notices for requests
// that the old delegate started.
- (id)delegate;
...
// Should the socket operate in SSL server mode or client mode?  The default is
// client mode (NO).  If you change this to YES, you must also call
// setSSLCertificates:.
- (BOOL)sslServerMode;
...
// requests complete.  Do not call this until the socket has successfully
// connected and you have set up any SSL options above. Also, do not call this
// until you are sure you have read all of the unencrypted data that the other
// party has sent, otherwise unencrypted data may be interpreted as SSL
// handshake data.  If the handshake fails, socket:sslHandshakeFailedWithError:
// will be called.  Otherwise, socketSSLHandshakeSucceeded: will be called.

// If the handshake fails, any data write requests sent after enableSSL is
// called will be transferred in the clear.  If it succeeds, they will be
// encrypted.  So if you are sending sensitive information, it is probably best
// to wait until the delegate method socketSSLHandshakeSucceeded: is called to
...
// Request that a specific amount of data be read and given to the delegate,
// timing out if it takes too long.  If timeout is negative, the read will never
// time out. userInfo can be anything you need to keep track of (or nil).  It
// will be retained and then released when the read fails or succeeds and is
// reported to the user.
- (void)readDataOfLength:(unsigned int)length
					timeout:(NSTimeInterval)timeout
					userInfo:(NSDictionary *)userInfo;

// Request that the socket read data until it hits the first instance of the
// given terminator, and then give the data read (including the terminator) to
// the delegate.  If timeout is non-negative, the read will time out if it takes
// longer than timeout seconds.  If it is negative, it will never time out.
// userInfo can be anything you need to keep track of (or nil).  It will be
// retained and then released when the read fials or succeeds and is reported to
// the user.
- (void)readUntilData:(NSData *)terminator
				timeout:(NSTimeInterval)timeout
				userInfo:(NSDictionary *)userInfo;

// Use this method to retrieve any available data.  It will be put into the
// queue of read requests just like the other style of reading, and when any
// previous read requests have finished it will return any received data that is
// left or an empty NSData instance if there is none.
...
// Request that the given data be written to the socket and the success or
// failure of the operation be reported to the delegate.  The write will time
// out if it takes too long.  If timeout is negative, the write will never time
// out.  userInfo can be anything you need to keep track of (or nil).  It will
// be retained and then released when the write fails or succeeds and is
// reported to the user.
